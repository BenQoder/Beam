...
type BeamGlobalConfig = {
  __BEAM_DISABLE_AUTO_CONNECT__?: boolean
  __BEAM_AUTO_CONNECT__?: boolean
}
...
function shouldAutoConnect(): boolean {
  const globals = globalThis as BeamGlobalConfig
  if (globals.__BEAM_DISABLE_AUTO_CONNECT__) return false
...
function maybeConnect(reason: string): void {
...
const api = {
  ...
  async visit(url: string, options: VisitOptions = {}): Promise<VisitResponse> {
    const session = await ensureConnected()
    return session.visit(url, options) as unknown as VisitResponse
  },
...
function getMatchingWrapper(target: Element, html: string): MatchingWrapper | null {
  ...
  if (temp.childElementCount !== 1) return null
  ...
  if (targetId && rootId && targetId === rootId) return { root, innerHtml: root.innerHTML }

  const targetBeamId = target.getAttribute('beam-id')
  const rootBeamId = root.getAttribute('beam-id')
  if (targetBeamId && rootBeamId && targetBeamId === rootBeamId) return { root, innerHtml: root.innerHTML }

  const targetBeamItemId = target.getAttribute('beam-item-id')
  const rootBeamItemId = root.getAttribute('beam-item-id')
  if (targetBeamItemId && rootBeamItemId && targetBeamItemId === rootBeamItemId) return { root, innerHtml: root.innerHTML }

  return null
}

function syncWrapperAttributes(target: Element, wrapper: Element): void {
  if (!(target instanceof HTMLElement) || !(wrapper instanceof HTMLElement)) return
...
function collectPreservedNodes(target: Element, keepSelectors: string[] = []): PreservedNode[] {
  ...
  const shouldPreserve = (el: Element): boolean => {
    ...
    if (el.hasAttribute('beam-island') && !el.hasAttribute('beam-island-remount')) return true
...
function restorePreservedNodes(target: Element, preserved: PreservedNode[]): void {
  for (const { selector, el } of preserved) {
    ...
    if (el.hasAttribute('beam-island')) {
      ...
      if (placeholder.getAttribute('beam-island') !== el.getAttribute('beam-island')) continue
 ...
 */
function applyIslandPropsResponse(updates: Record<string, unknown>): void {
 ...
 */
function applyIslandUpserts(upserts: Record<string, IslandUpsertSpec>): void {
  Object.entries(upserts).forEach(([id, spec]) => {
    ...
    if (spec.src) el.setAttribute('beam-island-src', spec.src)
    if (spec.load && spec.load !== 'eager') el.setAttribute('beam-island-load', spec.load)
...
function applyIslandRemovals(ids: string[]): void {
  ids.forEach((id) => {
    const el = document.querySelector(`[beam-island][beam-id="${escapeCss(id)}"]`)
    if (!el) {
...
function getVisitMode(link: HTMLAnchorElement): VisitMode {
  if (link.hasAttribute('beam-patch')) return 'patch'
  if (link.hasAttribute('beam-navigate')) return 'navigate'
  return 'visit'
}

function getVisitElementSelector(el: Element | null): string | null {
  if (!el) return null
...
function getVisitTargetSelector(link: HTMLAnchorElement): string {
  const explicitTarget = link.getAttribute('beam-target')
  if (explicitTarget) return explicitTarget
  const boostRoot = link.closest('[beam-boost]')
  if (!boostRoot || boostRoot === link) return 'body'
  return getVisitElementSelector(boostRoot) ?? 'body'
}

function isLocalHttpLink(link: HTMLAnchorElement): boolean {
  return link.origin === location.origin && (link.protocol === 'http:' || link.protocol === 'https:')
}

function isBeamVisitLink(link: HTMLAnchorElement): boolean {
  if (link.hasAttribute('beam-boost-off')) return false
...
function getVisitCacheKey(url: string, mode: VisitMode, target: string): string {
...
function getManagedHeadKey(node: Element): string | null {
...
function syncManagedHead(doc: Document): void {
...
function focusAfterVisit(targetSelector: string): void {
  const autofocusEl = document.querySelector<HTMLElement>('[autofocus]')
  if (autofocusEl) {
    autofocusEl.focus()
    return
  }
  ...
  const target = targetSelector === 'body'
    ? document.querySelector<HTMLElement>('main, [role="main"], body')
...
function applyVisitScroll(response: VisitResponse): void {
  if (response.scroll === 'preserve') return
...
function commitVisitHistory(url: string, mode: VisitMode, target: string, replace: boolean): void {
...
async function prefetchVisit(link: HTMLAnchorElement): Promise<void> {
  if (!isBeamVisitLink(link) || !isLocalHttpLink(link)) return
  ...
  if ((cached && cached.expires > Date.now()) || visitPreloading.has(key)) return
...
document.addEventListener('click', async (e) => {
  ...
  if (!link) return
  if (!isBeamVisitLink(link)) return
  if (!isLocalHttpLink(link)) return
...
type PollingState = {
  ...
  reader?: ReadableStreamDefaultReader<ActionResponse>
}
const pollingElements = new Map<HTMLElement, PollingState>()

function startPolling(el: HTMLElement): void {
  if (pollingElements.has(el) || !el.hasAttribute('beam-poll') || !el.getAttribute('beam-action')) return
  ...
  const poll = async () => {
    if (!active()) { if (pollingElements.get(el) === state) stopPolling(el); return }
    try {
      if (!isOnline) return
      const action = el.getAttribute('beam-action')
      if (!action) { stopPolling(el); return }
      ...
      try {
        while (active()) {
          ...
          if (!target && !value.target && value.html) {
            ...
            if (applyResponse({ ...value, html: undefined }, null, mode, el)) { stopPolling(el); break }
          } else if (applyResponse(value, target, mode, el)) { stopPolling(el); break }
        }
      } finally {
        if (!active()) await reader.cancel().catch(() => {})
        state.reader = undefined
        reader.releaseLock()
      }
    } catch (err) {
      if (active()) handleActionError({ action: el.getAttribute('beam-action') || undefined, message: err instanceof Error ? err.message : String(err) })
    } finally {
      // Schedule after the response stream completes: never overlap calls.
      if (active()) state.timer = setTimeout(poll, interval())
      else if (pollingElements.get(el) === state) stopPolling(el)
    }
  }
  if (el.hasAttribute('beam-poll-delay')) state.timer = setTimeout(poll, interval())
  else void poll()
}
...
function stopPolling(el: HTMLElement): void {
  ...
  if (!el.hasAttribute('beam-poll') || !document.body.contains(el) || !el.getAttribute('beam-action')) el.removeAttribute('beam-poll-observed')
}

const pollMutationObserver = new MutationObserver(() => {
  for (const el of pollingElements.keys()) {
    if (!document.body.contains(el) || !el.hasAttribute('beam-poll') || !el.getAttribute('beam-action')) stopPolling(el)
  }
  document.querySelectorAll<HTMLElement>('[beam-poll]:not([beam-poll-observed])').forEach((el) => {
    if (!el.getAttribute('beam-action')) return
...
async function manualVisit(url: string, options: VisitOptions = {}): Promise<void> {
