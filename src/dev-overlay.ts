...
function pushError(error: OverlayError): void {
  errors.push(error)
  render()
}

function start(): void {
  const marker = globalThis as { __BEAM_DEV_OVERLAY_ACTIVE__?: boolean }
  if (marker.__BEAM_DEV_OVERLAY_ACTIVE__) return
  marker.__BEAM_DEV_OVERLAY_ACTIVE__ = true

  window.addEventListener('beam:action-error', (e) => {
    const detail = (e as CustomEvent<{ action?: string; message: string; stack?: string }>).detail
    if (!detail) return
    pushError({
  ...
  window.addEventListener('beam:island-error', (e) => {
    const detail = (e as CustomEvent<{ name?: string; src?: string | null; phase?: string; error?: unknown }>).detail
    if (!detail) return
  ...
  window.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && document.getElementById(OVERLAY_ID)) hideOverlay()
  })
}
