...
function processReactiveBindings(root, _state) {
    ...
    root.querySelectorAll('[beam-init]').forEach((el) => {
        if (!isInReactiveScope(el, root))
            return;
        if (el.hasAttribute('beam-init-run'))
            return;
...
function ensureNamedState(id, initial) {
...
export const beamReactivity = {
     ...
     */
    ensureState: (id, initial) => ensureNamedState(id, initial),
    /**
