...
function ensureDevSecret(cwd, result, options) {
...
function ensureDevVarsIgnored(cwd, result, options) {
    const file = join(cwd, '.gitignore');
    const current = existsSync(file) ? readFileSync(file, 'utf8') : '';
    if (current.split(/\r?\n/).some((line) => line.trim() === '.dev.vars'))
        return;
    if (!result.changed.includes('.gitignore'))
        result.changed.push('.gitignore');
    if (!options.dryRun) {
