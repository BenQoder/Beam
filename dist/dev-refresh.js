...
function rootSelector() {
    const explicit = document.querySelector('[beam-dev-refresh-root]');
    if (explicit?.id)
        return `#${cssEscape(explicit.id)}`;
    if (explicit)
        return '[beam-dev-refresh-root]';
    const boost = document.querySelector('[beam-boost]');
    if (boost?.id)
        return `#${cssEscape(boost.id)}`;
    return 'body';
}
...
function preserveAndSwap(target, source) {
...
function morphNode(target, source) {
    ...
    if (target.hasAttribute('beam-keep'))
        return;
...
function notifyAfterMorph(root) {
...
if (!globalThis.__BEAM_DISABLE_DEV_REFRESH__) {
    startDevRefresh();
}
